  ...
  return (
    <section className="space-y-6">
      <header className="flex items-start justify-between gap-4">
        <div>
          <h1 className="font-serif text-4xl text-[var(--forest)]">Contact Page</h1>
          <p className="mt-2 text-[var(--muted)]">Edit your Contact page content and SEO.</p>
        </div>
        <Link
          href="/contact"
          target="_blank"
      ...
      <form action={updateHomepageContent} className="rounded-2xl border border-[var(--line)] bg-white p-6 space-y-4">
        <input type="hidden" name="page_slug" value="contact" />

        <div className="grid gap-4 md:grid-cols-2">
          <label className="space-y-1 text-sm block">
            <span className="font-medium text-[var(--ink)]">Page title</span>
            <input name="title" defaultValue={page?.title || "Contact"} className="w-full rounded-lg border border-[var(--line)] px-3 py-2" />
          </label>
          <div />
          <label className="space-y-1 text-sm block md:col-span-2">
            ...
            <textarea
              ...
              rows={10}
              defaultValue={body}
              placeholder="Contact information and enquiry details…"
        ...
        <button className="rounded-full bg-[var(--forest)] px-5 py-2 text-sm font-semibold text-white">Save Contact page</button>
      </form>
    </section>
  );
}
